...
func TestCreatePGAuditConfigQuery(t *testing.T) {
	var roleName = "foo"
	var parameterKey = "pgaudit.log"
	...
	for _, c := range cases {
		out := createPGAuditConfigQuery(schema.TestResourceDataRaw(t, resourcePostgreSQLPGAuditConfig().Schema, c.resource))
		if out != c.expected {
...
func TestResetRoleQuery(t *testing.T) {
	...
	for _, c := range cases {
		out := createResetPGAuditConfigQuery(schema.TestResourceDataRaw(t, resourcePostgreSQLPGAuditConfig().Schema, c.resource))
		if out != expected {
...
func TestAccPostgresqlPGAuditConfig(t *testing.T) {
	skipIfNotAcc(t)
	...
	testAccPostgresqlPGAuditConfigResources := fmt.Sprintf(`
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccPostgresqlPGAuditConfigResources,
				Check: resource.ComposeTestCheckFunc(
					...
					checkPGAuditConfig(t, dsn, roleName, parameterKey, parameterValue),
				),
			},
		},
	})
...
func checkPGAuditConfig(t *testing.T, dsn, role string, parameterKey string, parameterValue string) resource.TestCheckFunc {
